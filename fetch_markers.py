...
def fetch_and_store_priority_component_files(test_dir, output_folder):
    ...
    # Dictionary to map TCIDs to a set of filenames
    tcid_dict = defaultdict(set)
    # Dictionary to map each unique Component to filenames
    component_dict = defaultdict(set)
    # Dictionary to map each unique Priority to filenames
    priority_dict = defaultdict(set)
    # Set to store all unique components and priorities
    all_components = set()
    all_priorities = set()
    # Set to store all unique TCIDs
    all_tcids = set()
    ...
    for root, _, files in os.walk(test_dir):
        for file in files:
            if file.endswith(".py"):
                ...
                # Add components and priorities to their respective sets
                all_components.update(components)
                ...
                # Map each Component to the file
                for component in components:
                    component_dict[component].add(file)

                # Map each Priority to the file
                for priority in priorities:
                    priority_dict[priority].add(file)
    ...
    # Write all unique priorities to priorities.txt
    priorities_file = os.path.join(output_folder, "Priorities.txt")
    print(f"Creating priorities file: {priorities_file}")
    ...
    # Write filenames to separate files based on each Component
    for component, files in component_dict.items():
    ...
    # Write filenames to separate files based on each Priority
    for priority, files in priority_dict.items():
        formatted_priority = priority.capitalize()  # Capitalize priority
        priority_file = os.path.join(output_folder, f"{formatted_priority}.txt")
